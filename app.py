...
def get_avg_inst_freq(df_filter, sfreq, Options, **kwargs):# [trial_duration,sfreq,epoch_duration]=[30,2000,1]
    ...
    for c,ch in enumerate(Options['selected_channels']):# c,ch = 0,Options['selected_channels'][0] # c,ch = c+1,Options['selected_channels'][c+1]
        ...
        slope_norm = (((t_epoch[ch][-1]*slope + intercept)-(t_epoch[ch][0]*slope + intercept))/(t_epoch[ch][0]*slope + intercept))/trial_duration# change in AIF/start value of AIF/trial duration in seconds, yielding percent change in AIF per second
        intercept_norm = 1
        fig_aif.add_trace(
            go.Scatter(
                y=avg_inst_freq[ch],
    ...
    # #     ax.set_title(str(labels[a]))
    # #     ax.legend()
    # # axes[0].set_ylabel('Frequency ($Hz$)')
    # # axes[1].set_ylabel('Relative Frequency (%)')
    # # axes[0].legend(loc='upper right')
    # # axes[1].legend(loc='upper right')
    # for c,ch in enumerate(Options['selected_channels']):# c,ch = 0,Options['selected_channels'][0]
...
def get_median_freq(df_filter, sfreq, Options, **kwargs):
    ...
    # fig_mdf = plt.figure('MDF regressions',figsize=(16,8))
    # axes = fig_mdf.subplots(2,1,sharex=True)# ,sharex='col',sharey='row')#
    ...
    for c,ch in enumerate(Options['selected_channels']):# c,ch = 0,Options['selected_channels'][0] # c,ch = c+1,Options['selected_channels'][c+1]
        ...
        slope_norm = (((t_epoch[ch][-1]*slope + intercept)-(t_epoch[ch][0]*slope + intercept))/(t_epoch[ch][0]*slope + intercept))/trial_duration# change in MDF/start value of MDF/trial duration in seconds, yielding percent change in MDF per second
        intercept_norm = 1
        # axes[0].plot(
        #     t_epoch[ch],
        #     median_freq[ch],
        ...
        # axes[0].plot(
        #     t_epoch[ch],
        #     t_epoch[ch]*slope + intercept,
        #     label='linear '+labels[c],
        #     )
        # axes[1].plot(
        #     t_epoch[ch],
        #     median_freq[ch]/intercept,
        #     label='MDF norm '+labels[c]
        #     )
        # axes[1].plot(
        #     t_epoch[ch],
        #     t_epoch[ch]*slope_norm + intercept_norm,
        #     label='linear norm '+labels[c]
        #     )
        fig_mdf.add_trace(
            go.Scatter(
                y=median_freq[ch],
...
def get_mean_freq(df_filter, sfreq, Options, **kwargs):
    ...
    # fig_mnf = plt.figure('MNF regressions',figsize=(16,8))
    # axes = fig_mnf.subplots(2,1,sharex=True)# ,sharex='col',sharey='row')#
    ...
    for c,ch in enumerate(Options['selected_channels']):# c,ch = 0,Options['selected_channels'][0]
        ...
        for i,on in enumerate(ons):# i,on = 0,ons[0]
            ...
            t_epoch[ch][i] = central_point / sfreq
            # Processing window power spectrum (PSD) generation
            # freqs, power = signal.periodogram(np.array(processing_window),
            #                            fs=sfreq)# plt.plot(freqs, power)
            f, Pxx_den = signal.periodogram(np.array(processing_window), fs=float(sfreq))# plot_freq_spectrum(f, Pxx_den)
        ...
        slope_norm = (((t_epoch[ch][-1]*slope + intercept)-(t_epoch[ch][0]*slope + intercept))/(t_epoch[ch][0]*slope + intercept))/trial_duration# change in MNF/start value of MNF/trial duration in seconds, yielding percent change in MNF per second
        intercept_norm = 1
        # axes[0].plot(
        #     t_epoch[ch],
        #     mean_freq[ch],
        ...
        # axes[1].plot(
        #     t_epoch[ch],
        #     mean_freq[ch]/intercept,
        #     label='MNF norm '+labels[c]
        #     )
        # axes[1].plot(
        #     t_epoch[ch],
        #     t_epoch[ch]*slope_norm + intercept_norm,
